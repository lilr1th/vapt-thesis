...
LOGO_FIGURES = {
    ...
    "7":  (os.path.join(IMGS, "pentest_process.png"),             "Figure 7: Penetration testing process diagram"),
    ...
    "11": (os.path.join(IMGS, "nmap_logo.png"),                 "Figure 11: Nmap network scanner logo"),
    "12": (os.path.join(IMGS, "nikto_logo_fixed.png"),          "Figure 12: Nikto web server scanner logo"),
    "13": (os.path.join(IMGS, "shodan_logo_fixed.png"),         "Figure 13: Shodan internet-connected device search engine logo"),
    "14": (os.path.join(IMGS, "burpsuite_logo.png"),            "Figure 14: Burp Suite Community Edition logo"),
    "15": (os.path.join(IMGS, "cyberchef_logo.png"),            "Figure 15: CyberChef data analysis tool logo"),
...
LOGO_WIDTHS = {
    "2":   15.0,   # org chart
    "3":   15.0,   # office map
    "7":   15.0,   # pentest flow diagram
    "5.1": 15.0,   # recommended architecture
}
...
def add_front_matter(doc):
    ...
    body(doc, "Secondly, I would like to express my appreciation to **MR. LAY Heng, Head of Department of Information and Communication Engineering (GIC) at ITC**. His guidance and management have provided excellent education and courses within my department which are the foundation of this internship.")
    body(doc, "Thirdly, I would like to give my deep gratitude to my supervisor, **DR. KUY Movsun, Lecturer at the Department of Information and Communication Engineering (GIC), ITC**. His kindness and dedication of time have guided this internship to achieve its goals. From the beginning of the internship to the final report, his thoughtful suggestions were invaluable.")
    ...
    lof_entries = [
        ...
        ("2",    "Prestige Alliance organisational structure"),
        ("3",    "Prestige Alliance office location map"),
        ("4",    "Hack The Box platform logo"),
        ("5",    "HTB Certified Penetration Testing Specialist (CPTS) logo"),
        ("6",    "HTB Certified Web Exploitation Specialist (CWES) logo"),
        ("7",    "Penetration testing process diagram"),
        ("8",    "OWASP logo"),
        ("9",    "NIST SP 800-115 logo"),
        ("10",   "CVSS v3.1 scoring system logo"),
        ("11",   "Nmap network scanner logo"),
        ("12",   "Nikto web server scanner logo"),
        ("13",   "Shodan search engine logo"),
        ("14",   "Burp Suite Community Edition logo"),
        ("15",   "CyberChef data analysis tool logo"),
        ("16",   "Claude (Anthropic) AI assistant logo"),
...
def process_md(doc):
    ...
    while i < len(lines):
        ...
        if m_img5:
            ...
            if num in SHOT_FIGURES:
                path, cap = SHOT_FIGURES[num]
                add_img(doc, path, cap, w=LOGO_WIDTHS.get(num, 14.0))
            else:
                path = os.path.join(IMGS, fname)
                add_img(doc, path, f"Figure {num}: {desc}", w=LOGO_WIDTHS.get(num, 13.0))
            i+=1; continue
